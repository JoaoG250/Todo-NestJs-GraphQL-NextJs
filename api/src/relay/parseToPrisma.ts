...
import { cursorToIdentifier } from "./connectionArray";
...
export function parsePaginationArgs(args: ConnectionArguments) {
  try {
...
function relayToPrismaPagination(args: ConnectionArguments): {
  cursor?: { id: string };
  take?: number;
  skip?: number;
} {
...
function resolveCursor(
  ...
  if (before) {
    return { id: cursorToIdentifier(before) };
  }

  if (after) {
    return { id: cursorToIdentifier(after) };
  }
